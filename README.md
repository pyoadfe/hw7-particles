...
> **В данном задании необходимо использовать версию `tensorflow==2.15` (поддерживает Python 3.11 или ниже) и `tf2onnx==1.16.1`.** Это связано с тем, что в пакет `tf2onnx` пока не добавили поддержку более новых версий `tensorflow`. Для установки зависимостей проще всего использовать прилагающийся файл `requirements.txt`:
> ```
